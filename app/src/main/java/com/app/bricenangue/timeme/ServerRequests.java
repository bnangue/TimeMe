...
public class ServerRequests {
   ...
   // public static final String SERVER_ADDRESS="http://time-tracker.comlu.com/";
    ...
    public void fetchUserForRequestinBackground(String email, GetUserCallbacks callbacks){
        progressDialog.setCancelable(false);
        ...
        progressDialog.show(activity.getSupportFragmentManager(), "task_progress");
        lockScreenOrientation(activity);
        new FectchUserForRequestAsynckTacks(email,callbacks).execute();
    }

    public void fetchUserForFCMNotificationinBackground(String email, GetUserCallbacks callbacks){
        progressDialog.setCancelable(false);
        progressDialog.setTitle(activity.getSupportFragmentManager(),activity.getString(R.string.AsyncTsk_Saving_Email_Password_Changed_User_In_Backgroung));

        progressDialog.show(activity.getSupportFragmentManager(), "task_progress");
        lockScreenOrientation(activity);
        new FectchUserForFCMNotificationAsynckTacks(email,callbacks).execute();
    }
    ...
    public void updateFrienListInBackgroung(User user,String email,GetUserCallbacks callbacks){
        progressDialog.setCancelable(false);
        progressDialog.setTitle(activity.getSupportFragmentManager(),activity.getString(R.string.AsyncTsk_Upadte_Grocery_List_In_Backgroung));

        progressDialog.show(activity.getSupportFragmentManager(), "task_progress");
        lockScreenOrientation(activity);
        new UpdateFriendListAsynckTacks(user,email,callbacks).execute();
    }
            ...
            try {
               ...
               urlConnection.setConnectTimeout(CONNECTION_TIMEOUT);
                ...
                String data =URLEncoder.encode("email","UTF-8")+"="+URLEncoder.encode(user.email,"UTF-8")+"&"+
                        ...
                        +"&"+
                        URLEncoder.encode("gcmregid","UTF-8")+"="+URLEncoder.encode(String.valueOf(user.regId),"UTF-8");
                buff.write(data);
                buff.flush();
                buff.close();
                out.close();
    ...
    public class FectchUserForRequestAsynckTacks extends AsyncTask<Void,Void,User> {
        String email;
        GetUserCallbacks userCallbacks;

        public FectchUserForRequestAsynckTacks(String email, GetUserCallbacks callbacks) {
            this.email = email;
            this.userCallbacks = callbacks;
        }
        ...
        protected User doInBackground(Void... params) {
            ...
            HttpURLConnection urlConnection=null;
            try {
                url=new URL(SERVER_ADDRESS + "FetchUserForFriendRequest.php");
                urlConnection=(HttpURLConnection)url.openConnection();
                urlConnection.setReadTimeout(CONNECTION_TIMEOUT);
                urlConnection.setConnectTimeout(CONNECTION_TIMEOUT);
                urlConnection.setRequestMethod("POST");
                ...
                if(jsonObject.length()==0){
                    ...
                    if(jsonObject.has("email")){
                        ...
                        returneduser=new User(email,password,firstname,lastname,stat,regId,bitmap,friendlist);
                    }
    ...
    public class FectchUserForFCMNotificationAsynckTacks extends AsyncTask<Void,Void,User> {
        String email;
        GetUserCallbacks userCallbacks;

        public FectchUserForFCMNotificationAsynckTacks(String email, GetUserCallbacks callbacks) {
            this.email = email;
            this.userCallbacks = callbacks;
        }
        ...
        protected User doInBackground(Void... params) {
            ...
            HttpURLConnection urlConnection=null;
            try {
                url=new URL(SERVER_ADDRESS + "FetchUserForFCMNotification.php");
                urlConnection=(HttpURLConnection)url.openConnection();
                urlConnection.setReadTimeout(CONNECTION_TIMEOUT);
                urlConnection.setConnectTimeout(CONNECTION_TIMEOUT);
                urlConnection.setRequestMethod("POST");
    ...
    public class UpdateFriendListAsynckTacks extends AsyncTask<Void,Void,String>{

        User user;
        String email;
        GetUserCallbacks callbacks;

        public UpdateFriendListAsynckTacks(User user, String email,GetUserCallbacks callbacks){
            this.user=user;
            this.callbacks=callbacks;
            this.email=email;
        }
        ...
        protected String doInBackground(Void... params) {
            ...
            data.add(new Pair<String, String>("currentEmail",user.email));
            data.add(new Pair<String, String>("currentPassword", user.password));
            ...
            try {
                ...
                byte[] postData= getData(data).getBytes("UTF-8");
                url=new URL(SERVER_ADDRESS + "UpdateFriendLists.php");
                urlConnection=(HttpURLConnection)url.openConnection();
                urlConnection.setReadTimeout(CONNECTION_TIMEOUT);
                urlConnection.setConnectTimeout(CONNECTION_TIMEOUT);
                urlConnection.setRequestMethod("POST");
    ...
    public class createGroceryAndUpdateFinanceAccountAsynckTacks extends AsyncTask<Void,Void,String>{
        ...
        protected String doInBackground(Void... params) {
            ...
            data.add(new Pair<String, String>("list_isShareStatus",String.valueOf(shareStatus)));
            data.add(new Pair<String, String>("list_note","nothing specify"));
            data.add(new Pair<String, String>("list_account_id",groceryList.getAccountid()));
    ...
    public ArrayList<GroceryList> getGroceryListdetails(JSONArray jsonArray){
        ...
        try {
            ...
            for (int i = 0; i < jsonArray.length(); i++) {
                ...
                String listaccountid = jo_inside.getString("list_account_id");
                ...
                groceryList.setAccountid(listaccountid);
